...
对于 CPU 密集型任务，使用`multiprocessing`替代`threading`是传统的解决方案。
...
Python 3.13 引入了实验性的自由线程模式（Free-threading），可以通过编译时选项`--enable-free-threaded`启用，这将完全移除 GIL：
