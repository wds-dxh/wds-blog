# docker概述 
## 什么是Docker容器？
Docker容器是一种轻量级的虚拟化技术，它允许开发者将应用及其依赖文件封装在Docker镜像文件中，然后在不同的物理设备上运行这些镜像，从而实现应用的快速部署和资源的高效利用[1](https://it.sohu.com/a/722022828_121161532)。
...
![](./assets/1732765280397-bfb4851b-b22d-4bb5-be35-543d831ec17e.png)
...
![](./assets/1729178746814-2271ca1c-d173-4611-afdf-bf4014c6b5a8.png)

### docker inspect 
+ 查看容器的元数据

![](./assets/1729178778926-34aea635-fda0-4ac6-b883-5dca86e19ef5.png)
...
### 直接在容器中使用命令来挂载数据卷
通过 `docker run` 命令使用 `-v` 参数，可以将宿主机的目录挂载到容器内。具体命令格式如下：
...
在返回的 JSON 中，查找 `Mounts` 字段，确保你挂载的路径被正确映射。

### 匿名挂载和具名挂载
#### 匿名挂载---容器删除自动删除volume
匿名挂载不指定卷的名字，Docker 会自动为你创建一个匿名卷，并挂载到容器内的指定路径。

```bash
docker run -d -P --name nginx01 -v /etc/nginx nginx
```

+ 这条命令将宿主机的目录 `/etc/nginx` 挂载到容器中的 `/etc/nginx` 目录，但它是匿名挂载，即没有指定卷的名称。

#### 具名挂载
具名挂载需要给数据卷指定一个名称，这样你可以方便地管理和复用它。

```bash
docker run -d -P --name nginx02 -v nginxconfig:/etc/nginx nginx
```

+ 这条命令将创建一个名为 `nginxconfig` 的数据卷，并将它挂载到容器中的 `/etc/nginx` 目录。

#### 查看卷的信息：
可以通过 `docker volume inspect` 查看具名卷的详细信息，确认挂载的是卷名而不是宿主机目录名。

```bash
docker volume inspect nginxconfig
```

+ 如果挂载的是卷名，返回的 `Mountpoint` 字段通常会显示 Docker 内部的路径，例如 `/var/lib/docker/volumes/nginxconfig/_data`。
+ 如果挂载的是宿主机目录，`Mountpoint` 会显示宿主机的路径。
...
### 继承父容器的数据卷
有时你需要从另一个容器继承数据卷。`--volumes-from` 允许你从一个已存在的容器中继承数据卷。
...
+ 这种方式常用于创建多个容器之间共享数据，尤其是需要多个容器同时使用同一份数据时。
...
![](./assets/1729317394093-16abe9b4-5c3f-4fde-9778-96761a1bf17b.png)
...
![](./assets/1729328341083-9dbb4c6c-d159-49ec-bf83-a351e65e357d.png)

#### 每启动一个容器，linux主机就会多了一个虚拟网卡。  
 使用了veth-pair 技术！  

![](./assets/1729329124691-8d6f4253-7004-4b95-aa1c-dafccda06b18.png)

#### 容器间能够相互ping通，不过是使用的docker0作为路由器
#### 网络模型	  
![](./assets/1729329494016-9a49efc3-d1cb-43ab-8d9c-e93da36a4641.png)
...
![](./assets/1729330904088-decd8599-bb1b-4d73-960d-45a41acee7c0.png)
...
![](./assets/1729331333559-28900bb1-3b4e-4a6c-a535-56d01f1740c3.png)
